...
daia_capability_exists() {
    case "$1" in
        desktop.environment | window.manager | graphics.stack | network.manager | network.stack)
            return 0
            ;;
        *)
            return 1
...
daia_registry_plugin_exists() {
    case "$1" in
        desktop/environment | desktop/window-manager | system/graphics-stack | system/network-manager | system/network-stack)
            return 0
            ;;
        *)
            return 1
...
daia_registry_plugin_metadata() {
    ...
    case "$plugin_id:$field" in
        ...
        system/graphics-stack:provides)
            printf '%s\n' 'graphics.stack'
            ;;
        system/network-manager:provides)
            printf '%s\n' 'network.manager'
            ;;
        system/network-stack:provides)
            printf '%s\n' 'network.stack'
            ;;
        *)
            return 1
...
daia_plugin_registry_plugin_dependencies() {
    case "$1" in
        ...
        system/network-manager)
            printf '%s\n' 'system/network-stack'
            ;;
        system/graphics-stack | system/network-stack)
            return 0
            ;;
        *)
            return 1
